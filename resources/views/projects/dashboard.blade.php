<x-app-layout>
    ...
    <div x-data="{ activeTab: 'tasks' }" class="flex h-full">
            ...
            <div x-show="activeTab === 'finance'" x-transition.opacity>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                     ...
                     <div class="bg-white rounded-lg shadow p-4 flex items-center">
                         ...
                         <div class="ml-3"> <h4 class="text-xs font-medium text-gray-500 uppercase">Total Estimasi Gaji</h4> <p class="text-xl font-semibold">Rp {{ number_format($financialStats['totalHakGaji'], 0, ',', '.') }}</p> </div>
                     </div>
                     <div class="bg-white rounded-lg shadow p-4 flex items-center">
                         ...
                         <div class="ml-3"> <h4 class="text-xs font-medium text-gray-500 uppercase">Total Dibayar</h4> <p class="text-xl font-semibold">Rp {{ number_format($financialStats['totalPaid'], 0, ',', '.') }}</p> </div>
                     </div>
                     @php
        ...
        {{-- Right Sidebar - Informasi Proyek & Tim (Tetap Sama) --}}
        <div class="hidden md:block w-80 lg:w-96 bg-gray-50 border-l border-gray-200 overflow-y-auto flex-shrink-0">
           {{-- Konten Sidebar Kanan Persis seperti sebelumnya --}}
            <div class="p-6 space-y-6">
                <div> <h3 class="text-base font-semibold mb-3 text-gray-800">Informasi Proyek</h3>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                if (taskStatusCtx) {
                    ...
                    new Chart(taskStatusCtx, {
                        type: 'doughnut', data: { labels: ['Todo', 'In Progress', 'Review', 'Done'], datasets: [{ label: 'Jumlah Tugas', data: [ taskData.todo, taskData.in_progress, taskData.review, taskData.done ], backgroundColor: ['#E5E7EB', '#FCD34D', '#93C5FD', '#6EE7B7'], borderColor: ['#9CA3AF', '#F59E0B', '#3B82F6', '#10B981'], borderWidth: 1, hoverOffset: 4 }] },
                        options: { responsive: true, maintainAspectRatio: false, cutout: '60%', plugins: { legend: { position: 'bottom', labels: { padding: 15, boxWidth: 12 } } } }
                    });
                } else { console.warn("Canvas #taskStatusChart not found."); }
                ...
                if (tasksByAssigneeCtx) {
                    ...
                    new Chart(tasksByAssigneeCtx, {
                        type: 'bar', // Tipe Bar chart
                        data: {
                            labels: assigneeStatusData.labels, // Nama assignee
                            datasets: assigneeStatusData.datasets // Sudah berisi data per status
                        },
                        options: {
                            ...
                            scales: {
                                x: { stacked: true, beginAtZero: true, ticks: { precision: 0 } }, // Sumbu X ditumpuk
                                y: { stacked: true } // Sumbu Y ditumpuk
                            },
                            plugins: {
                                legend: { position: 'bottom', labels: { padding: 15, boxWidth: 12 } },
                                tooltip: {
                                    mode: 'index', // Tampilkan tooltip untuk semua stack pada index yg sama
                                    intersect: false,
                                    callbacks: {
                ...
                } else { console.warn("Canvas #tasksByAssigneeStatusChart not found."); }
                ...
                if (financialCtx) {
                    ...
                    new Chart(financialCtx, {
                        type: 'doughnut', data: { labels: [ 'Task Dibayar', 'Lainnya Dibayar', 'Estimasi Belum Dibayar' ], datasets: [{ label: 'Jumlah (Rp)', data: [ finData.paidTask, finData.paidOther, finData.remainingUnpaid ], backgroundColor: ['#6EE7B7', '#A7F3D0', '#FCD34D'], borderColor: ['#10B981', '#34D399', '#F59E0B'], borderWidth: 1, hoverOffset: 4 }] },
                ...
                } else { console.warn("Canvas #financialOverviewChart not found."); }
                ...
                if (spendingCtx) {
                    ...
                    new Chart(spendingCtx, {
                        type: 'bar', data: { labels: ['Status Keuangan'], datasets: [ { label: 'Budget', data: [spendingData.budget], backgroundColor: '#BFDBFE', borderColor: '#60A5FA', borderWidth: 1 }, { label: 'Estimasi Gaji', data: [spendingData.hakGaji], backgroundColor: '#FEF3C7', borderColor: '#FBBF24', borderWidth: 1 }, { label: 'Sudah Dibayar', data: [spendingData.paid], backgroundColor: '#A7F3D0', borderColor: '#34D399', borderWidth: 1 } ] },
                ...
                } else { console.warn("Canvas #spendingVsBudgetChart not found."); }
