...
class PaymentController extends Controller
{
    ...
    public function storePayslip(Request $request, Project $project)
    {
        ...
        if ($paymentTypeInput === 'task') {
            $specificRules = [
                'task_ids' => ['present', 'array'], // 'present' memastikan field ada, bisa kosong jika tidak required
                'task_ids.*' => ['integer', 'exists:tasks,id'], // Pastikan setiap ID adalah integer dan ada di tabel tasks
            ];
            // Jika task_ids WAJIB diisi untuk tipe 'task'
            // $baseRules['task_ids'] = ['required', 'array', 'min:1'];
        } elseif ($paymentTypeInput === 'termin') {
            $specificRules = [
                'payment_term_id' => ['required', 'integer', Rule::exists('payment_terms', 'id')->where('project_id', $project->id)],
                ...
                $validTasksCollection = $tasksFromRequest->filter(function ($task) use ($validated, $project, $paymentTypeInput, $startDate, $endDate) {
                    ...
                    if (!$isBasicValid) {
                        Log::warning("storePayslip - Task {$task->id} basic validation failed: ", [
                            'task_project_id' => $task->project_id, 'expected_project_id' => $project->id,
                            'task_assigned_to' => $task->assigned_to, 'expected_user_id' => $validated['user_id'],
                            'task_status' => $task->status, 'task_payment_id' => $task->payment_id
                        ]);
                        return false;
                    }
